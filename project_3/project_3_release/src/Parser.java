...
//	Cell[][] table2 = new Cell[][];
	...
	private ArrayList<Entry> buildLargerParses(Cell left, Cell right) {
		...
		ArrayList<Entry> leftcell = new ArrayList<Entry>();
		ArrayList<Entry> rightcell = new ArrayList<Entry>();
		leftcell = left.getParses();
		rightcell =right.getParses();
		for(int i=0; i<leftcell.size();i++){
			for(int j=0; j<rightcell.size();j++){
				String sss = leftcell.get(i).lhs + " " +rightcell.get(j).lhs;
				if(g.findLHS(sss) != null){
					for(String stringsss:g.findLHS(sss)){
					largerParses.add(new Entry(stringsss,leftcell.get(i),rightcell.get(j)));
					}
				}
			}
		}
	...
	public void parse(ArrayList<String> sentence) {
		...
		for (int i = 0; i < numWords; i++) {
			...
			for (int column = 0; column < (numWords - i); column++) {
				row.add(new Cell());
			}
			table.add(row);
		}
		...
		for (int i = 0; i < numWords; i++) {
			Cell rowstart = table.get(i).get(0);
			String words = sentence.get(i);
			ArrayList<String> lhs = g.findPreTerminals(words);
			for (String words_preteminal : lhs) {
				Entry diagonalentry = new Entry(words_preteminal, words);
				rowstart.add(diagonalentry);
			}
		}

		for (int column = 1; column < numWords; column++) {
			for (int n = 1; n <= column; n++) {
				int m = column - n;
				Cell cellfinal = table.get(m).get(n);
				for (int i = 0; i < n; i++) {
					Cell left = table.get(m).get(i);
					Cell right = table.get(m + i + 1).get(n - i - 1);
					ArrayList<Entry> cellfinalentry = buildLargerParses(left, right);
					for (Entry e : cellfinalentry) {
						cellfinal.add(e);
					}
				}
			}
	...
	public static void main(String[] args) {
		...
		*/				                /*"with",
		...
		try {
			...
			//FileReader isReader = new FileReader("./results/sentence.txt");
